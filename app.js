...
app.use((req, res, next) => {
    ...
    res.locals.serviceName = 'GMI Mail';
    res.locals.serviceDomain = 'gmindonusa.com';
...
app.use((req, res, next) => {
    if (
        ...
        !['/account/logout', '/account/start-u2f', '/account/check-u2f', '/account/check-totp', '/account/send-otpsms', '/account/check-otpsms', '/account/verify-otpsms'].includes(req.url.split('?').shift())
    ) {
        return passport.csrf(req, res, err => {
            ...
            if (req.session.require2fa) {
                if (req.session.require2fa.includes('otpsms')) {
                    apiClient['2fa'].sendOtpsms(req.user, req.session.id, req.ip, (err, result) => {
                        if (err) {
                            return next(err);
                        }
            ...
            return res.render('account/2fa', {
                ...
                enabledOtpsms: req.session.require2fa ? req.session.require2fa.includes('otpsms') : false,
