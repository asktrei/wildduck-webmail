...
module.exports = {
    ...
    mailApproval: {
        list(user, callback) {
            _exec('get', '/users-disabled', false, user.token, (err, response) => {
                if (err) {
                    return callback(err);
                }
                return callback(null, (response && response.results) || []);
        ...
        approve(user, callback) {
            _exec('post', '/user-disabled/{user}', { user: requestBody }, user.token, callback);
        },

        del(user, sess, ip, callback) {
            _exec('del', '/user-disabled/{user}?ip={ip}', { user: requestBody, sess, ip }, false, user.token, callback);
        }
    },

    '2fa': {
        setupOtpsms(user, ip, callback) {
            _exec('post', '/users/{user}/2fa/otpsms/setup', { user: user.id }, { ip }, user.token, callback);
        },

        verifyOtpsms(user, sms, sess, ip, callback) {
            _exec('post', '/users/{user}/2fa/otpsms/enable', { user: user.id }, { sms, sess, ip }, user.token, callback);
        },

        checkOtpsms(user, token, sess, ip, callback) {
            _exec('post', '/users/{user}/2fa/otpsms/check', { user: user.id }, { token, sess, ip }, user.token, callback);
        },
        
        sendOtpsms(user, sess, ip, callback) {
            _exec('post', '/users/{user}/2fa/otpsms/send', { user: user.id }, { sess, ip }, user.token, callback);
        },
