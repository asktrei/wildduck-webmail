...
const AUTH_EVENTS = new Map([
    ...
    ['enable 2fa Otpsms', 'Enable 2FA SMS authenticator'],
    ['disable 2fa Otpsms', 'Disable 2FA SMS authenticator'],
    ['verify 2fa Otpsms', 'Authenticating with SMS authenticator'],
    ...
    ['approve mail', 'Approve registered mail']
    // ['reject mail', 'Create new Application Specific Password']
]);
...
router.get('/', (req, res) => {
    res.render('account/security/2fa', {
        ...
        enabledOtpsms: req.user.enabled2fa ? req.user.enabled2fa.includes('otpsms') : false,
...
router.get('/mail-approval', (req, res) => {
    apiClient.mailApproval.list(req.user, (err, response) => {
        if (err) {
            req.flash('danger', 'Email account was approved');
            res.redirect('/account/security');
        }
        res.render('account/security/mail-approval', {
            title: 'Security',
            activeSecurity: true,
            secMenuMailApproval: true,

            values: req.user,

            csrfToken: req.csrfToken(),

            mailApproval: mailApproval.reverse().map((entry, i) => {
                entry.index = i + 1;
...
router.post('/mail-approval/delete', (req, res) => {
    apiClient.mailApproval.del(req.user, req.body, req.session.id, req.ip, err => {
        if (err) {
            req.flash('danger', 'Database Error, failed to delete email account');
            return res.redirect('/account/security/mail-approval');
        }

        req.flash('success', 'Email account was deleted');
        return res.redirect('/account/security/mail-approval');
    });
});

router.post('/mail-approval/approve', (req, res) => {
    apiClient.mailApproval.approve(req.user, req.body, (err, response) => {
        if (err) {
            req.flash('danger', err.message);
            return res.redirect('/account/security');
        }

        req.flash('success', 'Email account was approved');
        return res.redirect('/account/security/mail-approval');
    });
});
...
router.post('/2fa/enable-otpsms', (req, res, next) => {
    ...
    let showErrors = errors => {
        apiClient['2fa'].setupOtpsms(req.user, req.ip, (err, data) => {
            if (err) {
                return next(err);
            }
            res.render('account/security/enable-otpsms', {
                layout: 'layout-popup',
...
router.post('/2fa/verify-otpsms', (req, res) => {
    const authSchema = Joi.object().keys({
        sms: Joi.string()
            .length(6)
            .required()
    ...
    apiClient['2fa'].verifyOtpsms(req.user, req.body.sms, req.session.id, req.ip, err => {
        if (err) {
...
router.post('/2fa/disable-otpsms', (req, res, next) => {
