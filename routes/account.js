...
const dialIntl = require('../public/dial-international-code.json');
let recaptcha;
...
router.get('/create', (req, res, next) => {
    ...
    // let phoneCountryID = config.service.pcid[0];

    res.render('account/create', {
        title: 'Create new account',
        activeCreate: true,
        domains: config.service.domains,
        // phoneCountryIDs: config.service.pcid,
        values: {
            domain,
            // phoneCountryID
        },
...
router.post('/create', recaptchaVerify, (req, res, next) => {
    ...
    const createSchema = {
        ...
        password2: Joi.string()
            ...
            .required(),
        phoneCountryID: Joi.string()
            .trim()
            // .valid(config.service.pcid)
            // .regex(/^[+][0-9]{1,3}$/)
            // .label('Country ID')
            .required(),
        phone: Joi.string()
    ...
    if (
        ...
        (['abuse', 'administrator', 'hostmaster', 'majordomo', 'root', 'ssl-admin', 'webmaster'].includes(result.value.username) ||
    ...
    apiClient.users.create(
        {
            ...
            middleName: (result.value.middleName) ? result.value.middleName : "",
            lastName: (result.value.lastName) ? result.value.lastName : "",
            gender: result.value.gender,
            birthDate: result.value.birthDate,
            phoneCountryID: "+"+dialIntl.find(el => el.code === result.value.phoneCountryID)["callingCode"].toString(),
            phone: (result.value.phone.charAt(0) == 0) ? result.value.phone.slice(1) : result.value.phone,
...
router.post('/profile', passport.checkLogin, (req, res) => {
    const updateSchema = Joi.object()
        .keys({
            ...
            birthDate: Joi.string().empty().label('Birth Date'),
            phoneCountryID: Joi.string().label('Region'),
            phone: Joi.string()
                .trim()
               // .regex(/^[0-9]{7,11}$/)
                .label('Phone'),
    ...
    result.value.middleName = result.value.middleName || '';
    result.value.lastName = result.value.lastName || '';
    result.value.birthDate = result.value.birthDate || '';
    result.value.phoneCountryID = result.value.phoneCountryID || '';
    result.value.phone = result.value.phone || '';
...
router.post('/check-otpsms', (req, res) => {
    ...
    apiClient['2fa'].checkOtpsms(req.user, result.value.token, req.session.id, req.ip, (err, result) => {
        if (err) {
            return res.json({ error: err.message, code: err.code });
        }
